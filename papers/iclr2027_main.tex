...
\begin{abstract}
Agent-to-Agent (A2A) protocols let autonomous agents collaborate on complex tasks, but reliable consensus in heterogeneous agent networks remains a fundamental challenge once Byzantine failures and soft faults are admitted. We propose \textbf{A2A-BFT}, a Byzantine fault-tolerant consensus extension for A2A protocols: a three-phase semantic consensus protocol (Propose--Validate--Commit) with fault-adaptive decision thresholds and a reputation tracker excluded by construction from the vote-counting rule. We prove safety and liveness under $n \geq 3f + s + 1$ ($f$ Byzantine, $s$ soft faults, $s \le f$) and evaluate entirely on real LLM inference: a 7{,}750-task fault sweep ($n{=}250$ per cell) with four heterogeneous open-source models, spanning mathematical reasoning, code generation, and knowledge QA. On execution-validated code tasks the protocol committed no wrong answer in any within-boundary cell ($0.0 \pm 0.0\%$ under every attack; per-cell $95\%$ Wilson upper bound $1.5\%$), while sub-boundary settings violate safety up to $72.4\%$ on GSM8K and $90.0\%$ on knowledge QA, consistent with $3f{+}s{+}1$ being a tight operating condition for $s \le f$; on semantically validated tasks, wrong commits within the boundary are bounded by the validation oracle's error rate ($11.6$--$23.2\%$ on GSM8K, $35.6$--$60.8\%$ on MMLU, with 8B judges). Under collusion, unvalidated majority-voting and debate baselines commit incorrect answers in $16.7$--$63.4\%$ of tasks \emph{within} the boundary. Ablations identify view change as the core liveness component---removing it collapses decisions from $74.4\%$ to $4.5\%$ under strategic rejection (three-seed ablation)---and fault-adaptive thresholds as the safeguard against attack-induced deadlock. A measured audit shows the released reputation tracker to be decision-neutral but mis-specified as a deterrent: in the four attacked cells, $1{,}186$ of $1{,}305$ aggressive penalties landed on honest validators whose vote was objectively correct.
\end{abstract}
...
\textbf{Motivating Example:} five agents must agree on a code solution while one is compromised (Byzantine fault) or intermittently erroneous (soft fault). Traditional BFT protocols assume deterministic, byte-verifiable replicas; LLM-based agents are \emph{probabilistic} replicas with no deterministic verifier, so agreement becomes a semantic problem outside classical BFT theory, requiring new mechanisms.

Recent work by ETH Zurich \citep{a2a_sim_2026} found that LLM-based agents reach valid consensus in only 41.6\% of trials \emph{even in benign settings without any Byzantine agents}, with failures dominated by liveness loss rather than value corruption. This gap motivates our work: how can a consensus protocol achieve reliability in heterogeneous agent networks whose participants may be Byzantine, soft-faulted, or heterogeneous in capability?

\textbf{Our Approach:} We propose A2A-BFT, a Byzantine fault-tolerant consensus extension with three innovations: (i) a three-phase semantic consensus (Propose-Validate-Commit) tailored to LLM-based reasoning tasks; (ii) fault-adaptive decision thresholds that prevent attack-induced deadlock (Section~\ref{sec:experiments}); and (iii) formal safety and liveness guarantees under partial synchrony with the fault model $n \geq 3f + s + 1$ (with the standing assumption $s \leq f$). We validate the protocol entirely on real four-model LLM deployments (7{,}750-task fault sweep, $n{=}250$ per cell), showing that (a) execution-validated consensus is mechanically safe within the boundary and violations below it match the theorem's prediction, and (b) on semantically validated tasks, wrong commits under attack are bounded by the validation oracle's error rate (Corollary~\ref{cor:oracle}). We additionally isolate the reputation tracker by direct measurement: replaying instrumented vote streams shows it decision-neutral, as the design requires, but, as released, it directs $1{,}186$ of $1{,}305$ aggressive penalties in the attacked cells at honest validators whose votes were objectively correct.

\section{Related Work}

\textbf{Byzantine fault tolerance in distributed systems.} Classical BFT protocols such as PBFT~\citep{castro1999practical} assume deterministic, faithfully executing replicas with bounded, known-size faults; Byzantine-robust aggregation in distributed learning~\citep{blanchard2017byzantine} tolerates adversarial gradient contributions but targets numeric aggregation. A2A-BFT instead targets \emph{probabilistic} replicas whose correctness depends on sampled LLM outputs, requiring semantic rather than byte-level validation and a unanimity-style agreement probability $p_h^{\,n-1-f-s}$ (Section~\ref{sec:theory}). The $+s$ term is not a re-badged omission count---an omission fault goes silent and is discountable, whereas a soft fault keeps voting and may err stochastically; Section~\ref{sec:theory} encodes this via coefficient $1$ on $s$ (vs.\ $3$ on $f$), worst-case abstention in the safety proof, and the standing assumption $s \leq f$.

\textbf{Multi-agent consensus with LLMs.} Debate- and voting-based aggregation improves answer quality but offers no guarantee against committed wrong answers under Byzantine agents: our experiments show $16.7$--$63.4\%$ wrong-commit rates for such baselines. Self-aggregation in the A2A-Sim style stops by supermajority vote and has no validation stage. Verifier-guided generation and reranking methods \citep{gsm8k,lightman2024verify} likewise use verification to \emph{rank or filter} candidates; A2A-BFT instead uses it to \emph{gate} commitment. No prior LLM-consensus protocol, to our knowledge, states a wrong-commit bound in terms of the validation oracle's error rate; A2A-BFT makes that dependence explicit (Corollary~\ref{cor:oracle}) and measures it. Concurrent robust-aggregation designs offer neither property: confidence-probe weighting \citep{zheng2025rethinking} keeps unvalidated weighted aggregation, and self-anchored filter-and-refine consensus \citep{lee2026robust} commits the majority-filtered answer without a validation oracle. The gap is not hypothetical: even an oracle-assisted debate baseline---keeping any solution that passes all tests, under the same round budget---still commits wrong answers on $46.7$--$56.7\%$ of MBPP tasks under attack (Appendix~\ref{app:attacks}). It is validation \emph{inside} the commit rule, not robustness of the aggregation weights, that binds wrong commits to the oracle's error rate. Interoperability standards (MCP, ANP) are orthogonal transport fabrics beneath this consensus layer (extended discussion: Appendix~\ref{app:extrelated}).

\section{System Model}

\subsection{Agent Network Model}

We consider a network of $n$ heterogeneous agents $\mathcal{A} = \{A_1, A_2, \ldots, A_n\}$ operating under the A2A protocol framework. Each agent $A_i$ has a unique decentralized identifier (DID) for authentication, a capability score $g_i$ reflecting its domain expertise, a reputation score $r_i \in [0.1, 1.0]$ updated dynamically, and a participation weight $w_i \in [0.1, 2.0]$ used only when the deployment subsamples validators (not in the vote-counting rule; Section~\ref{sec:reputation}).
...
A2A-BFT operates in rounds, each consisting of three phases as illustrated in Figure \ref{fig:consensus_flow}: \textbf{Phase 1: Propose} (the designated primary generates a task solution), \textbf{Phase 2: Validate} (other agents independently verify the proposal), and \textbf{Phase 3: Commit} (agents vote on the verification result; the count score $\phi$ is compared against fault-adaptive thresholds).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\linewidth]{figures/consensus_flow.pdf}
\caption{A2A-BFT three-phase consensus protocol. The round-robin primary broadcasts a proposal $\pi$ (Phase~1); validators re-solve and verify it---error-free checks for deterministic tasks and $m$-verifier semantic checks otherwise (Phase~2); votes are aggregated by the count score $\phi(\pi)$ under fault-adaptive thresholds (Phase~3). Rejections trigger view change immediately, and PENDING outcomes do so after two consecutive rounds or a $2\Delta$ timeout.}
...
Verification re-computes the solution and compares results. Formally, writing $\hat{V}_i$ for validator $i$'s verdict on a proposal (the ground-truth oracle $V$ is defined in Section~\ref{sec:task}), let $\mathrm{ver}(T, s) \in \{0,1\}$ denote whether verification of solution $s$ for task $T$ succeeds: $\mathrm{ver}(T, s) = 1$ iff $s$ matches the ground truth (or an equivalent formulation) in the \emph{execution-validated} domain, where the check is error-free; in the \emph{semantically validated} domain (free-form reasoning and multiple-choice QA), $\mathrm{ver}(T, s) = 1$ iff at least $k$ of $m$ verifiers agree \citep{zheng2023llm}, with $k = \lceil 2m/3 \rceil$; the verifier error then falls to the binomial tail $\sum_{j=0}^{k-1}\binom{m}{j}p_h^{\,j}(1-p_h)^{m-j}$ rather than $(1-p_h)^m$, where $p_h$ is the probability that an honest validator correctly accepts a valid proposal. Symmetrically, $p_{\mathit{mr}}$ denotes the oracle's \emph{miss} probability---accepting or abstaining on a wrong proposal; safety (Theorem~\ref{thm:safety}) is governed by this second error direction. In our experiments code generation (MBPP) is execution-validated while mathematics (GSM8K) and knowledge QA (MMLU) are semantically validated, each validator using a \emph{single} self-model judge ($m{=}1$), so redundancy comes \emph{across} validators via Phase~3; a mild self-preference bias is possible when a validator shares the primary's model \citep{zheng2023llm}; with four models across five replicas such pairings occur regularly (a residual bias source). The released judge emits a discrete verdict, ABSTAIN marking an explicit UNCERTAIN outcome (the band $[0.4, 0.6]$ on a calibrated probability); an ABSTAIN contributes 0 to $\phi(\pi)$.

This tail decays exponentially only once $k/m$ is held fixed: the ceiling in $k = \lceil 2m/3\rceil$ makes small-$m$ choices non-monotone ($0.028$ at $m{=}3$ but $0.0815$ at $m{=}5$ for $p_h = 0.9$), so deployments should pick $m$ as a multiple of three; $m > 1$ trades inference cost for lower verification error.
...
When a proposal is rejected or times out, secondary validators may submit competing proposals, which enter the PENDING state for consideration next round, with priority to higher-confidence submissions.
...
\label{thm:safety}
Under $n \geq 3f + s + 1$, DID-authenticated channels, single-proposal broadcast with consistent dissemination of honest votes, and worst-case soft-fault abstention without false-positive ACCEPTs on invalid proposals (Definition~\ref{def:fault}), A2A-BFT satisfies: (i) \emph{decision consistency}: no two honest validators reach \emph{opposite} terminal decisions on the same proposal, and honest validators may differ only between ACCEPT and PENDING, which the commit broadcast or view change resolves; and (ii) \emph{commit validity}: within the boundary a wrong commit requires an honest validation error (or a wrong honest-primary proposal), whereas below the boundary Byzantine validators alone can commit a tampered proposal.
\end{theorem}

\begin{proof}[Proof sketch]
(i) Honest replicas share the same honest votes, so scores differ only through Byzantine vote equivocation, which the threshold gap provably absorbs ($\theta_{accept}-\theta_{reject} \geq 2f+0.5s > 1.5f$; Appendix~\ref{app:proofs}); opposite terminal decisions are therefore impossible, and ACCEPT-vs-PENDING divergence resolves via the commit broadcast or view change. Proposals carry the primary's DID signature, and conflicting signed proposals from one primary trigger a view change that voids the round, so equivocation cannot yield two concurrent acceptances. (ii) Byzantine validators number at most $f-1$ when the primary is Byzantine ($f$ otherwise) and contribute $+1$ each, so commitment requires at least $\theta_{accept}-(f{-}1) = n{-}3f{-}s \geq 1$ honest ACCEPT votes when the primary is Byzantine, and $\theta_{accept}-f = n{-}1{-}3f{-}s \geq 0$ otherwise. Within the boundary, then, a wrong proposal cannot be committed through Byzantine votes alone---it needs an honest validation error or a wrong honest-primary proposal, both governed by the oracle's error rates ($p_h$, $p_{\mathit{mr}}$; Section~\ref{sec:validate}). Below the boundary the implication fails instead through the honest validators' own REJECTs: a non-tampering primary's wrong proposal, rejected by $n{-}1{-}f{-}s$ honest validators (soft faults abstaining, the worst case) against $f$ Byzantine ACCEPTs, clears $\theta_{accept}$ exactly when $7f + 3s + 3 \geq 3n$, matching Table~\ref{tab:bft}. Full proof in Appendix~\ref{app:proofs}. \qed
\end{proof}

\begin{corollary}[Oracle composition]
\label{cor:oracle}
Within the boundary, every committed wrong answer implies a validation-oracle error (claim~(ii) of Theorem~\ref{thm:safety}): a false accept by an honest validator, or---at the exact boundary $\theta_{accept} = f$---a collective honest abstention carried by the $f$ Byzantine ACCEPTs. The wrong-commit rate is governed by the oracle's miss probability $p_{\mathit{mr}}$ (Section~\ref{sec:validate}) and is zero under a perfect oracle; measured on the released GSM8K vote streams, the honest validators' false-accept rate is $18.1\%$ (Wilson $95\%$ CI $[15.5, 21.2]$; Appendix~\ref{app:judge_calib}), the same order as the $11.6$--$23.2\%$ wrong-commit rates observed under attack (Table~\ref{tab:bft}).
\end{corollary}
...
\label{thm:liveness}
Under partial synchrony, $n \geq 3f + s + 1$, and the standing assumption $s \leq f$, A2A-BFT reaches a decision (ACCEPT or REJECT) for any valid task proposal with probability $1$---\emph{including} under sustained coordinated rejection of every honest proposal by the Byzantine validators. Coordinated rejection blocks termination only if soft faults vote REJECT (rather than abstain) and $f < s$.
\end{theorem}

\begin{proof}[Proof sketch]
The primary rotates round-robin, so an honest agent becomes primary within every $f+1$ rounds. With an honest primary, a sufficient event for acceptance is that all $(n-1-f-s)$ honest validators accept; Lemma~\ref{lem:acceptance} lower-bounds the single-round acceptance probability by $p_h^{(n-1-f-s)} = 0.81$ for $n{=}5, f{=}1, s{=}1$ ($p_h = 0.9$). This event survives coordinated rejection: all-honest-accept with soft faults abstaining (the worst-case convention of Theorem~\ref{thm:safety}) gives $\phi - \theta_{accept} = 0.5f \geq 0$ \emph{regardless of the Byzantine votes}. If the decision is PENDING, view change (requiring $2f+1$ confirmations) rotates the primary within $f+1$ further rounds. After GST, the probability of no acceptance within $k$ honest-primary rounds is bounded by $(1 - p_h^{(n-1-f-s)})^k$, so termination occurs with probability 1 as rounds grow; the bound is informative only for large $p_h$, and realized round-bounded rates are measured in Section~\ref{sec:experiments}. The assumption $f \geq s$ enters only when soft faults vote REJECT; if $f < s$, sustained coordinated rejection can block acceptance---the sole case outside the guarantee. Full proof in Appendix~\ref{app:proofs}. \qed
\end{proof}

\begin{lemma}[Acceptance Probability Bound]
\label{lem:acceptance}
For $n \geq 3f + s + 1$ and honest primary, the probability of acceptance in a single round is at least $p_h^{(n-1-f-s)}$, under conditional independence of validator errors given the proposal (an idealization: measured judge errors are model-correlated, Appendix~\ref{app:judge_calib}).
\end{lemma}

\begin{proof}
When the primary is honest, it generates a valid proposal. Each honest validator independently accepts with probability $p_h$. Since there are $(n-1-f-s)$ honest validators, the probability that all accept is $p_h^{(n-1-f-s)}$. All-accept ensures $\phi \geq \theta_{accept}$ under the stated abstention behavior of soft faults; if the $f$ Byzantine \emph{and} $s$ soft-fault validators all vote REJECT the implication requires $f \geq s$, as assumed throughout (Appendix~\ref{app:softfault}). \qed
\end{proof}

\subsection{Complexity Analysis}
Table \ref{tab:complexity} (Appendix~\ref{app:implementation}) compares message complexity with that of representative BFT protocols. A2A-BFT keeps $O(n^2)$ messages per round, like PBFT; measured rounds remain small ($2.6$--$4.5$, Table~\ref{tab:performance}) because fault-adaptive thresholds and view change restore progress under attack.
...
\textbf{Reproducibility:} All protocol-level fault-tolerance experiments share one heterogeneous four-model deployment with 5 seeds and 50 tasks per (configuration, seed, dataset), i.e., $n{=}250$ per cell (7{,}750 tasks over 31 cells); task sets are per-seed shuffles over GSM8K \citep{gsm8k}, MBPP \citep{mbpp}, and MMLU \citep{mmlu}, reused across configurations for paired comparison. \textbf{No simulation:} every reported number comes from real LLM inference ($7{,}750$ fault sweep + $4{,}320$ ablation/baseline + $770$ API tasks); seeds, models, hardware, and injection are specified in the Reproducibility Statement and Appendix~\ref{app:attacks}.

\textbf{Statistical Methodology:} results are reported as mean $\pm$ standard deviation over seeds with 95\% Wilson score intervals on pooled task counts; rounds are compared by two-sided Welch $t$-tests and decision rates by the exact McNemar test, both over identical per-task sampling; tests are confirmatory only for the pre-specified round-count and run-agreement comparisons of Section~\ref{sec:realapi}, all other between-cell comparisons are descriptive.

\subsection{Baseline Results}

Table \ref{tab:baseline} (Appendix~\ref{app:additional}) shows baseline performance without Byzantine faults.

\textbf{Note on decision semantics:} a task is \emph{decided} when a proposal is committed within the 6-round bound (rejections and PENDING rounds trigger view change and retry); \emph{wrong-commit rate} counts tasks committed with an incorrect answer. Under semantic validation the single-round acceptance bound $p_h^{\,n-1-f-s}$ is consistent with the $55$--$59\%$ fault-free decision rate on GSM8K---heterogeneous replicas disagree often, and the protocol refuses to commit when validation is inconclusive; on MBPP, where validation executes all test assertions, it is $70$--$74\%$ with \emph{zero} wrong commits.
...
\begin{table}[htbp]
\centering\small
\caption{BFT Performance Under Attack (real heterogeneous 4-model deployment; cells: decision\% / wrong-commit\%, mean$\pm$std over sampling seeds; $n{=}250$ per cell except the $n{=}5, f{=}1, s{=}1$ strategic-reject row, $n{=}90$, from Table~\ref{tab:ablation}, Appendix~\ref{app:additional}). Bold marks the \emph{best} (lowest) wrong-commit rate per column. $^\dag$: configuration below the safety boundary $3f{+}s{+}1$ ($n \geq 7$ required for $f{=}2$). $^{\ddag}$: smaller three-seed ablation deployment; not comparable to the $n{=}250$ rows. MMLU results in Table~\ref{tab:mmlu_sweep}.}
\label{tab:bft}
...
5 & 1 & 1 & Strat.\ Reject$^{\ddag}$ & $74.4 \pm 8.4$ & $\mathbf{4.5 \pm 3.9}$ & $70.0 \pm 11.5$ & $\mathbf{0.0 \pm 0.0}$ \\
...
Three findings emerge. \textbf{(i) Safety is exactly as strong as the validation oracle; the boundary is tight.} On MBPP, every configuration within the safety boundary commits a wrong answer in $\mathbf{0.0 \pm 0.0\%}$ of tasks ($n{=}250$/cell; zero events; per-cell $95\%$ Wilson upper bound $1.5\%$) regardless of attack, whereas all $n{=}5, f{=}2$ configurations below the boundary violate safety ($5.6$--$9.6\%$): $\theta_{accept} = 0$ lets two Byzantine ACCEPTs offset the honest REJECTs of a non-tampering primary's wrong proposal (the $n{=}6$ MBPP row stays at $0.0\%$: execution mechanically rejects tampered code, so its violation surfaces through text proposals; Appendix~\ref{app:proofs}). \textbf{(ii) On semantic domains, eventual acceptance trades safety for liveness under sustained attack.} GSM8K wrong commits rise to $11.6$--$23.2\%$ within the boundary: a Byzantine primary submits a plausible-but-wrong answer, weak 8B judges accept it at non-negligible rates, and retries sample until one passes validation---the system \emph{does} decide ($74$--$91\%$) but inherits the judge's error rate (held-out, two of the four MMLU judges accept wrong answers at $65$--$79\%$, Appendix~\ref{app:judge_calib}, consistent with Table~\ref{tab:mmlu_sweep}'s $35.6$--$60.8\%$). \textbf{(iii) Decision rates rise under attack} (at matched $n{=}5$ within the boundary, $89.2\%$ vs.\ $56.7\%$ fault-free; sub-boundary rows reach $98\%$): Byzantine primaries inject extra proposals that eventually clear validation; we therefore report wrong commits as the primary safety metric.

\subsection{Real LLM Validation (Homogeneous Upper Bound)}
\label{sec:realapi}

As a complementary check we validated the protocol on 320 real DeepSeek-V2-Lite tasks (GSM8K, MMLU, MBPP). These homogeneous single-model runs let honest validators agree far more often; the resulting $100\%$ acceptance across all eight configurations ($291$ of $320$ completed; $29$ API/harness failures excluded, Appendix~\ref{app:realllm}), under strategic rejection and collusion alike, is therefore an \emph{upper bound} on decision rates, not a contradiction of the $55$--$74\%$ heterogeneous results. At the boundary configuration $n{=}8, f{=}2, s{=}1$ (150 GSM8K tasks per configuration, real DeepSeek API), the protocol maintains $98$--$100\%$ consensus under both attacks across two independent runs, with answer correctness within $2$ points of the fault-free baseline ($92.0$--$92.7\%$ vs.\ $94.0\%$), and the round-count gap vs.\ fault-free ($1.03$ vs.\ $2.89$--$2.92$) confirms genuine attack resistance (Welch $t = -13.1$ and $-13.9$, $p < 10^{-26}$; reproduced at $1.04$ vs.\ $2.38$--$2.43$, with per-task decision agreement across runs, McNemar exact $p = 0.25$). Details, scaling, and MMLU sweeps: Appendices~\ref{app:realllm} and~\ref{app:additional}.

\subsection{Ablation Study}

To isolate each protocol component we ablate in the \emph{heterogeneous four-model} setting (Section~\ref{sec:setup} models, one per replica) on two domains with different validation semantics: \emph{GSM8K}, where Validate uses an independent LLM judge, and \emph{MBPP}, where Validate executes the submitted code against test assertions. Each scenario uses 30 tasks per seed over three seeds ($90$ each); Byzantine validators employ strategic rejection ($n{=}5, f{=}1, s{=}1$) or collusion ($n{=}8, f{=}2, s{=}1$). Soft faults exercise the abstention arm (ABSTAIN w.p.\ $0.3$; Appendix~\ref{app:softfault})---the stochastic-REJECT arm of Definition~\ref{def:fault} is analyzed, not injected (Table~\ref{tab:ablation}, Appendix~\ref{app:additional}, reports decision and wrong-commit rates).


The ablation yields four findings. \textbf{(i) View change is the core liveness component:} removing it collapses decisions from $74.4 \pm 8.4\%$ to $4.5 \pm 3.9\%$ (GSM8K, strategic rejection) and to $0\%$ (MBPP, both attacks)---blocked rounds never rotate the primary, confirming Theorem~\ref{thm:liveness}. \textbf{(ii) Semantic validation is the prerequisite for heterogeneous consensus:} under naive exact matching heterogeneous replicas almost never agree byte-for-byte and the code domain deadlocks ($0$--$1.1\%$ decisions). On GSM8K, dropping semantic validation \emph{lowers} wrong commits ($2.2 \pm 1.9\%$ vs.\ the full protocol's $4.5 \pm 3.9\%$)---as Corollary~\ref{cor:oracle} predicts: the exact-match oracle rarely commits and so rarely errs; the LLM-judge oracle commits more and inherits its error rate---a $20$-point decision-rate trade. \textbf{(iii) Fault-adaptive thresholds prevent attack-induced deadlock:} a fixed majority threshold drives every strategic-rejection round into PENDING ($0.0 \pm 0.0\%$), whereas fault-adaptive thresholds restore $74.4 \pm 8.4\%$. \textbf{(iv) Safety is oracle-bounded:} at most $10.0 \pm 3.3\%$ wrong commits on GSM8K and none on MBPP (per-cell $95\%$ Wilson interval $[0.0, 4.1]\%$ at $n{=}90$).

\textbf{Baselines under attack (Table~\ref{tab:hetero_compare}, Appendix~\ref{app:additional}; definitions: Appendix~\ref{app:attacks}).} Three observations (descriptive; three seeds per cell). \textbf{First, high decision rates are not safety.} All voting- and debate-based baselines decide on every task, yet under collusion commit incorrect answers in $16.7$--$63.4\%$---unvalidated aggregation converts Byzantine injections and honest errors into committed consensus; the full protocol forgoes them ($60$--$74\%$). \textbf{Second, unverified debate amplifies attacks:} LLM-Debate \citep{du2024improving} is the least safe on code ($63.4 \pm 5.8\%$ wrong commits under collusion), letting Byzantine agents drag honest replicas toward corrupted solutions. \textbf{Third, the correct-when-decided gap matters:} conditioned on committing, A2A-BFT reaches $84$--$100\%$ answer correctness in all eight attacked cells (Tables~\ref{tab:ablation},~\ref{tab:hetero_compare}) vs.\ $37$--$88\%$ for baselines; vs.\ A2A-Sim: GSM8K wrong commits within noise, but it decides fewer tasks and collapses on code ($34.4$--$35.5\%$ vs.\ our $0.0{\pm}0.0$).

\subsection{Reputation Tracker: Measured Behavior}\label{sec:rep_ablation}

The tracker cannot change a commit decision (Section~\ref{sec:reputation}); we measure \emph{what it reports}: logging every validation vote from the four-model deployment (120 runs, 457 scored rounds), we replay them offline through Algorithm~\ref{alg:reputation} under four settings (two $c$ sources $\times$ two reset policies; Appendix~\ref{app:rep_details}). Decision-neutrality is \emph{checked} rather than assumed: recomputing $\phi$ from the raw votes reproduces all $457$ recorded rounds; replaying the vote-and-threshold state machine reproduces all $120$ decisions; injecting reputation weights flips up to $5$ decisions, so the check can fail.

The measured behavior is negative. \textbf{(i) The aggressive tier fires on honest, correct votes:} across the four attacked cells, $1{,}186$ of $1{,}305$ aggressive penalties ($\rho_i \geq 0.7$) landed on honest validators whose votes were \emph{objectively correct} (the reward Section~\ref{sec:reputation} specifies)---and \emph{all 20 tasks} per cell had a mis-penalty. \textbf{(ii) As a Byzantine detector the tier fails:} precision $0.232$/$0.195$ (recall $1.000$) under strategic rejection but precision \emph{and} recall $0.000$ under collusion---colluders never reach $\rho = 0.7$; honest validators rejecting the same proposals do. \textbf{(iii) The score ordering inverts:} honest validators finish \emph{below} Byzantine ones in both collusion cells ($0.337$ vs.\ $0.427$ GSM8K, $0.300$ vs.\ $0.412$ MBPP), and per-task reset does not repair this ($0.391$ vs.\ $0.761$), so the failure is structural, not a cold-start artifact. \textbf{(iv) The released $c$ carries negative information:} \texttt{\_estimate\_correctness} is a per-domain constant (always \texttt{False} on GSM8K, \texttt{True} on MBPP) agreeing with ground truth on $46.2\%$ of rounds (trivial always-\texttt{False} predictor: $74.6\%$).

This failure mode is known \citep{josang2002beta,josang2007survey,kamvar2003eigentrust}; we add its scale in a real LLM deployment. No reported safety or liveness \emph{number} depends on the tracker; the Appendix~\ref{app:attack_resistance} deterrent is specified, not realized. A repaired trigger needs a proposal-quality oracle and a key untied to the validator's own rejection ratio (Appendix~\ref{app:rep_details}).



\textbf{Limitations.} A2A-BFT assumes DID-authenticated channels (Ed25519); fault-free decision rates ($55$--$59\%$ GSM8K, $11$--$15\%$ MMLU) show weaker semantic-validation reliability, and Theorem~\ref{thm:liveness}'s asymptotic guarantee is unreached within six rounds. Ablation cells ($n{=}90$) limit statistical power; the tracker is decision-neutral yet mis-specified (single seed).

\section{Conclusion}

We presented A2A-BFT, a Byzantine fault-tolerant consensus protocol for agent collaboration with provable safety and liveness under $n \geq 3f + s + 1$ ($s \leq f$). Real-LLM evaluation ($7{,}750$ tasks) shows liveness under every attack and theory-predicted safety: zero wrong commits on execution-validated tasks within the boundary, violations below it, oracle-bounded errors on semantic tasks. Two limits remain: the measured reputation tracker is mis-specified; realized liveness on weak oracles falls short (implications, privacy: Appendix~\ref{sec:discussion}).
\subsection*{AI Use Statement}
We used generative AI tools (large language model assistants) in the three roles described below. (i)~\emph{Software development}: they assisted in writing and debugging the implementation of A2A-BFT and of the baselines, the experiment harness that drives the real-LLM runs, the verification suite (ten independent auditors and five negative controls), and the scripts that regenerate the figures and the aggregate statistics. (ii)~\emph{Documentation}: they assisted in drafting and editing the documentation released with the artifact (repository README, reproduction guide, and audit notes). (iii)~\emph{Manuscript preparation}: they assisted in editing and polishing the language of this paper. No experimental data or result was generated by an AI tool: every number reported here is an output of the released scripts running real LLM inference, and the released audit suite recomputes it independently of this paper. Every AI-assisted artifact was executed and reviewed by the authors, who take responsibility for the final content of this work, including all text, claims, and artifacts.

\subsection*{Reproducibility Statement}
All source code, experimental data, and model configurations are available at \url{https://github.com/A2A-hub-ai/a2a-bft} (anonymized for review). Section~\ref{sec:setup} summarizes the setup; in full, all protocol-level fault-tolerance runs use the same heterogeneous four-model deployment with 5 seeds ($\{42, 43, 44, 45, 46\}$) and 50 tasks per (configuration, seed, dataset) ($n{=}250$ per cell; 7{,}750 tasks over 31 cells), with task sets drawn per seed by a seeded shuffle over GSM8K, MBPP, and MMLU and reused across configurations for paired comparison, while ablation and baseline comparisons use 3 seeds ($\{42, 43, 44\}$) with $n{=}90$ per cell. The four models (Llama-3.1-8B, DeepSeek-V2-Lite, InternLM3-8B, Qwen2.5-7B; Instruct/Chat variants) are served by vLLM \citep{vllm} $0.11$ (temperature $0$, max\_tokens $512$) on two NVIDIA A800-SXM4-80GB GPUs (CUDA 12.8, PyTorch 2.8.0). No simulated workers are used anywhere: every reported number comes from real LLM inference---7{,}750-task fault sweep; $4{,}320$ ablation/baseline tasks ($48$ cells $\times$ $90$); and $770$ API tasks ($320$ multi-domain validation, Table~\ref{tab:real_llm}, and $450$ adversarial-boundary scaling, Table~\ref{tab:n8_scaling}). Of the $48$ ablation/baseline cells, $36$ are tabulated (Tables~\ref{tab:ablation} and~\ref{tab:hetero_compare}); the remaining $12$ are fault-free baseline-condition pairings retained in the released aggregates. The reputation-tracker measurement of Section~\ref{sec:rep_ablation} additionally releases the fully instrumented vote streams (\texttt{reputation\_vote\_stream.json}) for 120 consensus tasks ($6$ cells $\times$ $20$), the two-phase measurement-and-replay script, the decision-neutrality audit (which recomputes the tally from raw votes, reproduces the recorded decisions, and mutation-tests its own resolution), and the fidelity audit against the released penalty code, so that every reported tracker quantity can be recomputed offline without any LLM calls; those quantities are event counts, reported without a variance.
...
\textbf{Note on sub-boundary configurations ($f{=}2$, $n \leq 6$):} The configurations $n = 5, f = 2, s = 0$ and $n = 6, f = 2, s = 0$ violate the theoretical safety boundary ($3f + s + 1 = 7 > n$). The real-LLM results (Table~\ref{tab:bft}) show exactly what the theorem predicts: with $\theta_{accept} = n - 1 - 2f - s \leq 1$ the honest majority no longer outvotes the Byzantine bloc ($f$ ACCEPT votes against $n{-}1{-}f{-}s$ honest REJECTs, soft faults abstaining, clear $\theta_{accept}$ whenever $7f + 3s + 3 \geq 3n$), and wrong-commit rates rise to $22.0$--$72.4\%$ (GSM8K) and $5.6$--$9.6\%$ (MBPP). Notably, on MBPP the $n{=}6, f{=}2$ collusion row still commits $0.0 \pm 0.0\%$ wrong answers: tampered code is rejected mechanically by execution, so the violation there surfaces through degraded thresholds interacting with plausible wrong \emph{text} proposals, not through code tampering. These rows empirically demonstrate that the boundary is tight, not a safety claim.
...
\end{equation}
honest ACCEPT votes when the primary is Byzantine (using $n \geq 3f+s+1$), and at least $\theta_{accept} - f = n-1-3f-s \geq 0$ when the primary is honest. So within the boundary a wrong proposal cannot be committed through Byzantine votes alone: it requires an honest validation error, or a wrong proposal from the honest primary itself---both governed by the oracle's error rates ($p_h$, $p_{\mathit{mr}}$; Section~\ref{sec:validate}). At the exact boundary ($\theta_{accept} = f$), all $f$ Byzantine ACCEPTs plus universal honest abstention also commit a wrong proposal ($\phi = f$)---a collective miss, charged to $p_{\mathit{mr}}$. Below the boundary this implication fails, but not through the Byzantine primary alone: non-Byzantine validators \emph{actively} reject ($-0.5$ each), so with $f{-}1$ Byzantine and $n{-}f$ non-Byzantine validators a tampering Byzantine primary reaches only $\phi = (f{-}1) - 0.5(n{-}f) < \theta_{accept}$. What the collapsed threshold does permit is a wrong proposal from a primary that does \emph{not} tamper, rejected by the $n{-}1{-}f{-}s$ honest validators (the worst case, with soft faults abstaining) while all $f$ Byzantine validators vote ACCEPT: then $\phi = f - 0.5(n{-}1{-}f{-}s) \geq \theta_{accept}$ exactly when $7f + 3s + 3 \geq 3n$; if soft faults REJECT alongside the honest validators the violation window shrinks to $7f + 2s + 3 \geq 3n$. For $n{=}5, f{=}2$ this reads $17 \geq 15$ (violation possible) and for $n{=}6, f{=}2$ it reads $17 \geq 18$ (violation impossible), matching the measured execution-validated wrong-commit rates of $5.6$--$9.6\%$ versus $0.0\%$ in Table~\ref{tab:bft}.

\textbf{Property 5: PENDING State Preservation.}
If $\theta_{reject} < \phi(\pi) < \theta_{accept}$, the decision is PENDING (matching Equation~\ref{eq:decision}, where $\phi \leq \theta_{reject}$ is a REJECT). In this case:
\begin{itemize}
...
Suppose a wrong proposal $\pi$ (i.e., $V(\pi)=0$ under ground truth) is committed within the boundary with an honest primary. Then the honest primary itself produced a wrong proposal, contributing an oracle error; or, with a Byzantine primary, Property 4 shows at least $n-3f-s \geq 1$ honest validators must have voted ACCEPT on $\pi$, i.e., at least one honest validation error occurred; or, at the exact boundary, all honest validators abstained on $\pi$ (a collective miss). In all three cases the wrong commit is attributable to the validation oracle rather than to the consensus layer, and its rate is bounded by the oracle's two-sided error probability ($p_h$, $p_{\mathit{mr}}$). Below the boundary the implication fails exactly as Property 4 predicts, which the sub-boundary rows of Table~\ref{tab:bft} confirm empirically. \qed
\end{proof}
...
When an honest agent is primary, it generates a valid proposal $\pi$. Honest validators verify $\pi$ independently. Let $p_h \geq 0.9$ be the probability that an honest validator correctly accepts a valid proposal (based on LLM accuracy; the measured value on the released GSM8K vote streams is $\approx 0.80$, Appendix~\ref{app:judge_calib}, which slows the geometric rate but does not affect the asymptotic statements). The probability that all $(n-1-f-s)$ honest validators accept is:
\begin{equation}
...
Sustained coordinated rejection of every honest proposal by the $f$ Byzantine validators does not escape the bound of Step 5: in an honest-primary round, the sufficient event of Step 3 (all honest validators accept) yields $\phi - \theta_{accept} = 0.5f \geq 0$ even when every Byzantine validator votes REJECT, provided soft faults abstain (the worst-case convention of Theorem~\ref{thm:safety}). The standing assumption $f \geq s$ extends this to soft faults voting REJECT ($\phi - \theta_{accept} = 0.5(f-s)$, Appendix~\ref{app:softfault}); if both $f < s$ and soft faults vote REJECT, the attack can block acceptance---the sole case outside Theorem~\ref{thm:liveness}. In deployments, view change rotates the primary within every $f+1$ rounds regardless, and where the reputation tracker is enabled (Algorithm~\ref{alg:reputation}) persistent rejecters are penalized ($-0.25$ per round once the reject ratio reaches $0.7$)---though Section~\ref{sec:rep_ablation} measures that the released tracker's aggressive tier in fact fires predominantly on honest validators, so this deterrent should be read as specified rather than realized. The ablation (Table~\ref{tab:ablation}) confirms the practical consequence, since removing view change collapses decision rates to $0$--$4.5\%$ under exactly this attack.
...
\begin{itemize}
    ...
    \item \textbf{A2A-Sim-style:} adapts the aggregation of \citet{a2a_sim_2026} to task answering---iterative answer proposals terminated by a supermajority-style agreement check---augmented with a $0.7$ confidence filter on countable proposals; no validation stage, no view change, no Byzantine-aware thresholds.
...
\midrule
Math baseline ($n=4,f=0,s=0$) & 40 & 100 & 6.30 \\
Math BFT ($n=5,f=1,s=1$) & 40 & 100 & 9.22 \\
Math + Strategic Reject ($n=5,f=1,s=1$) & 40 & 100 & 8.90 \\
Math + Collusion ($n=6,f=2,s=0$) & 40 & 100 & 8.49 \\
Knowledge baseline ($n=4,f=0,s=0$) & 40 & 100 & 27.00 \\
Knowledge BFT ($n=5,f=1,s=1$) & 40 & 100 & 28.13 \\
Code baseline ($n=4,f=0,s=0$) & 40\,(12)$^\star$ & 100 & 38.16 \\
Code BFT ($n=5,f=1,s=1$) & 40\,(39)$^\star$ & 100 & 9.01 \\
\midrule
...
\textbf{Note on Table~\ref{tab:real_llm}:} $^\star$The ``Tasks'' column shows submitted (and, in parentheses, completed) tasks: $28$ code-baseline tasks and $1$ code-BFT task terminated in API/harness errors, and $100\%$ is computed over the completed ones. All 320 tasks were submitted to the live DeepSeek API, and acceptance is computed over the $291$ that completed execution---every one was accepted. The other $29$ terminated in API/harness errors ($28$ in the code-baseline row, whose wall-clock is inflated to $38.16$s; $1$ in the code-BFT row) and are excluded; all $29$ are generation-layer transport failures, uncorrelated with the attack configuration (they occur in fault-free and attacked rows alike) and cannot affect the consensus verdicts, which are computed over completed tasks only. No proposal was \emph{rejected} in any configuration ($\texttt{reject\_count}=0$), so the $100\%$ column reflects consensus, not filtering, and every latency is a genuine end-to-end measurement---the sole caveat being the code-baseline average noted above, which those $28$ terminations inflate. This table is a homogeneous-deployment upper bound (Section~\ref{sec:realapi}); the authoritative adversarial-boundary results---real Byzantine tampering, numerical verification, measured rounds---are in Table~\ref{tab:n8_scaling}.

\textbf{Measuring Answer Correctness.} A consensus protocol can reach agreement on an answer that is nonetheless wrong. To validate the \emph{validity} (not merely the \emph{agreement}) guarantee of A2A-BFT, we additionally measure answer correctness: for each accepted proposal we extract the final numerical answer and compare it against the GSM8K ground truth. This distinguishes a protocol that merely terminates from one that terminates on the \emph{correct} answer. The results (Table~\ref{tab:n8_scaling}) show that under Byzantine attacks the consensus rate remains at or near $100\%$ ($98$--$100\%$ across two independent runs), while answer correctness stays within 2 percentage points of the fault-free baseline ($92.0$--$92.7\%$ vs.\ $94.0\%$). The decisive evidence of genuine resistance is the round-count gap ($1.03$ vs.\ $2.89$--$2.92$ rounds; Welch $t \leq -13.1$, $p < 10^{-26}$): Byzantine primaries tamper with answers, honest validators reject them, and view change rotates to an honest primary before a correct answer is accepted. This gap is reproduced in a repeat run ($1.04$ vs.\ $2.38$--$2.43$ rounds), whereas the consensus rate there is $100\%$; the paired per-task difference between the two runs is not significant (McNemar exact $p = 0.25$ on each attacked configuration, computed over the identical task sets), so we report the more conservative run and treat the residual decision-rate variation as API run-to-run variance.
...
\subsection{Judge Error-Rate Calibration from Released Vote Streams}\label{app:judge_calib}

The safety guarantee of Corollary~\ref{cor:oracle} is conditional on the validation oracle's miss probability $p_{\mathit{mr}}$. We measure it directly from the released per-round vote streams (Section~\ref{sec:rep_ablation}): every round records each validator's verdict together with the ground-truth correctness of the proposal under vote ($241$ of $254$ GSM8K rounds carry ground truth; $13$ empty-proposal rounds are excluded). Over all honest-validator votes on GSM8K---Byzantine and soft-fault voters excluded---the semantic judge accepts a wrong proposal with rate $18.1\%$ ($127/700$; $95\%$ Wilson CI $[15.5, 21.2]$) and rejects a correct proposal with rate $19.7\%$ ($53/269$; $[15.4, 24.9]$); the measured judge accuracy on correct proposals is thus $\approx 0.80$ rather than the idealized $p_h \geq 0.9$. Table~\ref{tab:judge_calib} breaks this down by scenario.

\begin{table}[htbp]
\centering\small
\caption{Measured honest-validator semantic-judge error rates on GSM8K, from the released per-round vote streams (honest voters only; empty-proposal rounds excluded). CI: Wilson $95\%$.}
\label{tab:judge_calib}
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{False accept (on wrong)} & \multicolumn{2}{c}{False reject (on correct)} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Scenario & rate (\%) & CI & rate (\%) & CI \\
\midrule
Fault-free ($n{=}5$) & $23.0$ & $[15.8, 32.2]$ & $27.6$ & $[21.2, 35.1]$ \\
Strategic reject ($n{=}5$, $f{=}1$, $s{=}1$) & $25.2$ & $[18.9, 32.6]$ & $10.2$ & $[4.4, 21.8]$ \\
Collusion ($n{=}8$, $f{=}2$, $s{=}1$) & $14.7$ & $[11.7, 18.3]$ & $7.8$ & $[3.4, 17.0]$ \\
\midrule
Overall & $\mathbf{18.1}$ & $[15.5, 21.2]$ & $\mathbf{19.7}$ & $[15.4, 24.9]$ \\
\bottomrule
\end{tabular}
\end{table}

Three observations. \textbf{First}, the measured false-accept rates are of the same order as the observed within-boundary wrong-commit rates under attack ($11.6$--$23.2\%$, Table~\ref{tab:bft}), closing the loop between the oracle-error account of Corollary~\ref{cor:oracle} and the end-to-end measurements. \textbf{Second}, at the measured $p_h = 0.80$ the single-round acceptance bound is $\geq 0.64$ for $n{-}1{-}f{-}s = 2$, so the asymptotic liveness guarantee (Theorem~\ref{thm:liveness}) is unaffected---only the geometric rate is slower than the $p_h = 0.9$ illustration. \textbf{Third}, these are \emph{on-protocol} measurements: proposals arise inside the consensus loop rather than from an independent held-out set. The held-out calibration below removes this caveat; MBPP is excluded from both because its execution validation is mechanically correct, and per-round MMLU vote streams were not logged (its higher wrong-commit rates, Table~\ref{tab:mmlu_sweep}, are consistent with a weaker semantic judge on that domain).

\textbf{Held-out calibration.} We additionally calibrate every judge on an independent held-out sample: $150$ GSM8K and $124$ MMLU tasks whose indices are excluded from every task set used in this paper (the full-sweep, ablation, reputation, and multiseed samplings), each presented to each judge model under two conditions---the ground-truth answer (measuring FNR) and a deterministic plausible perturbation, a numeric delta on GSM8K and a wrong choice letter on MMLU (measuring FPR)---using the protocol's judge prompts and verdict parsing verbatim, on the same four vLLM endpoints ($2{,}192$ calls; per-call records released as \texttt{heldout\_judge\_calibration.json}). Table~\ref{tab:judge_heldout} reports per-judge rates. Three findings. \emph{First}, FPR is dominated by a single judge: DeepSeek-V2-Lite accepts wrong answers at $73.3\%$ ($95\%$ CI $[65.3, 80.1]$) on GSM8K and $65.3\%$ on MMLU, whereas the other three judges reject almost all wrong GSM8K answers ($\leq 1.4\%$); decomposing the on-protocol vote streams by judge reproduces the same pattern (DeepSeek FPR $78.1\%$ on wrong proposals, the other judges $\leq 1.9\%$), so the $18.1\%$ aggregate of Table~\ref{tab:judge_calib} is a mixture of three strict judges and one permissive judge, not a uniform oracle error. \emph{Second}, held-out FNR ($5.1$--$15.2\%$ on GSM8K, $3.1$--$32.7\%$ on MMLU) is consistent with the on-protocol aggregate ($19.7\%$). \emph{Third}, InternLM3 often fails to emit a parseable verdict ($32\%$ of GSM8K and $73\%$ of MMLU calls; other judges $\leq 25\%$), which the protocol maps to ABSTAIN---an abstention, not an accept---so judge weakness degrades liveness, not safety. The implication is that deployed safety rests on the commit rule aggregating across heterogeneous judges: a validator pool dominated by permissive judges would inherit their FPR, so validator selection belongs in deployment checklists (Appendix~\ref{app:deployment}).

\begin{table}[htbp]
\centering\small
\caption{Held-out per-judge calibration (rate in \%, Wilson $95\%$ CI in brackets). Unparsed verdicts map to ABSTAIN and are excluded from the rates; InternLM3 abstains on $32\%$ (GSM8K) and $73\%$ (MMLU) of calls, other judges $\leq 25\%$.}
\label{tab:judge_heldout}
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{GSM8K} & \multicolumn{2}{c}{MMLU} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Judge & FPR & FNR & FPR & FNR \\
\midrule
Llama-3.1-8B & $1.4\,[0.4, 5.0]$ & $15.2\,[10.2, 22.1]$ & $37.6\,[28.8, 47.4]$ & $32.7\,[24.3, 42.3]$ \\
InternLM3-8B & $0.0\,[0.0, 3.8]$ & $7.5\,[3.9, 14.2]$ & $79.4\,[63.2, 89.7]$ & $3.1\,[0.6, 15.7]$ \\
DeepSeek-V2-Lite & $\mathbf{73.3}\,[65.3, 80.1]$ & $8.6\,[5.0, 14.5]$ & $\mathbf{65.3}\,[55.5, 74.0]$ & $31.3\,[22.9, 41.1]$ \\
Qwen2.5-7B & $1.4\,[0.4, 5.0]$ & $5.1\,[2.5, 10.2]$ & $24.5\,[16.9, 34.0]$ & $22.3\,[15.1, 31.8]$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Ablation Study under Byzantine Attacks (Heterogeneous 4-Model vLLM; mean$\pm$std over three sampling seeds, 30 tasks each, $n{=}90$ per cell; cells: decision\% / wrong-commit\%). Bold marks the \emph{highest} wrong-commit rate per column; columns where all variants tie at $0.0$ are left unmarked.}
\label{tab:ablation}
...
\midrule
Full protocol & $74.4{\pm}8.4$/$\mathbf{4.5{\pm}3.9}$ & $61.1{\pm}10.7$/$10.0{\pm}3.3$ & $70.0{\pm}11.5$/$0.0{\pm}0.0$ & $60.0{\pm}8.8$/$0.0{\pm}0.0$ \\
No View Change & $4.5{\pm}3.9$/$\mathbf{4.5{\pm}3.9}$ & $12.2{\pm}5.1$/$\mathbf{12.2{\pm}5.1}$ & $0.0{\pm}0.0$/$0.0{\pm}0.0$ & $0.0{\pm}0.0$/$0.0{\pm}0.0$ \\
No Sem.\ Validation & $54.4{\pm}8.4$/$2.2{\pm}1.9$ & $44.5{\pm}10.7$/$2.2{\pm}1.9$ & $1.1{\pm}1.9$/$0.0{\pm}0.0$ & $0.0{\pm}0.0$/$0.0{\pm}0.0$ \\
Fixed Threshold & $0.0{\pm}0.0$/$0.0{\pm}0.0$ & $38.9{\pm}7.7$/$0.0{\pm}0.0$ & $0.0{\pm}0.0$/$0.0{\pm}0.0$ & $51.1{\pm}13.4$/$0.0{\pm}0.0$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textbf{A2A-BFT (ours)} & $74.4/70.0/4.5$ & $61.1/51.1/10.0$ & $70.0/70.0/0.0$ & $60.0/60.0/0.0$ \\
Simple Majority & $100/71.1/\mathbf{28.9}$ & $100/66.7/\mathbf{33.3}$ & $100/43.3/\mathbf{56.7}$ & $100/38.9/61.1$ \\
Weighted Majority & $100/71.1/\mathbf{28.9}$ & $100/68.9/31.1$ & $100/51.1/48.9$ & $100/45.6/54.4$ \\
...
\subsubsection{Adversarial-Boundary Scaling \texorpdfstring{($n{=}8$, Real DeepSeek API)}{(n=8, Real DeepSeek API)}}
These homogeneous-API runs are an upper bound on decision rates (Section~\ref{sec:realapi}); Table~\ref{tab:n8_scaling} details answer correctness at the boundary configuration.
...
\caption{Real End-to-End Cost per Task (heterogeneous 4-model deployment): wall-clock time, LLM calls, and consensus rounds across six configurations---C1: $n{=}4$ baseline; C2: $n{=}4,f{=}1$ random; C3: $n{=}4,f{=}1$ strategic reject; C4: $n{=}5,f{=}1,s{=}1$ random; C5: $n{=}6$ baseline; C6: $n{=}5,f{=}2$ collusion (below the boundary). GSM8K CoT generation dominates latency; MBPP execution validation requires far fewer LLM calls.}
...
\begin{table}[htbp]
\centering
\caption{Attack Resistance Summary (real 4-model deployment; ranges over configurations of Table~\ref{tab:bft}, $n{=}250$ per cell, plus two $n{=}90$ cells: the within-boundary collusion configuration $n{=}8,f{=}2,s{=}1$ from Table~\ref{tab:hetero_compare} and the strategic-reject row $n{=}5,f{=}1,s{=}1$ marked~$^\ddag$ in Table~\ref{tab:bft}). ``Boundary'' = configurations satisfying $n \geq 3f{+}s{+}1$; ``below'' = $n{=}5,f{=}2$ and $n{=}6,f{=}2$ for $f{=}2$.}
\label{tab:attacks}
...
\textbf{Note:} Wrong-commit ranges span the two validation semantics: the lower end is always MBPP (execution validation, mechanically $0\%$ within the boundary) and the upper end is GSM8K (8B semantic judges). Within the boundary the protocol committed no wrong answer on execution-validated tasks; on semantic tasks the wrong-commit rate is bounded by the judge's susceptibility to plausible-but-wrong proposals. Below the boundary ($f{=}2$, $n{=}5,6$, where $3f{+}s{+}1 = 7 > n$) safety is violated on both domains (up to $9.6\%$ on execution-validated code and $72.4\%$ on GSM8K)---consistent with $3f{+}s{+}1$ being a tight operating condition in the evaluated regime $s \le f$.

\begin{itemize}
    \item \textbf{Strategic Rejection:} Specified to be penalized by the reputation tracker (Algorithm~\ref{alg:reputation}; Figure~\ref{fig:reputation}); the four-model sweep executes with uniform weights. Section~\ref{sec:rep_ablation} measures the released tracker as a Byzantine detector at precision $0.195$--$0.232$ under strategic rejection (recall $1.000$) but $0.000$ under collusion, with the bulk of aggressive penalties landing on honest validators whose vote was objectively correct (Section~\ref{sec:rep_ablation}), so the deterrent is best read as unrealized rather than merely unmeasured.
    \item \textbf{Collusion:} Within the boundary, collusion inflates wrong commits on semantic tasks exactly as the threshold analysis permits (Property~4; e.g., $17 \geq 15$ at $n{=}5, f{=}2$); on execution-validated tasks wrong commits remain $0\%$ because the commit rule requires passing the test suite. Below the boundary, collusion produces the largest violations in the sweep ($72.4\%$ on GSM8K). The thresholds bound \emph{when} safety can break, not \emph{whether} the validation oracle catches the injected proposals.
    \item \textbf{Sybil Attack:} DID-based identity authentication prevents identity replication, so an attacker cannot inflate its voting power by forging replicas. Because Equation~\ref{eq:commit} counts raw votes with uniform weights and reputation is excluded from the commit rule, REJECT-heavy or abstention-heavy behavior yields no additional weight; a Sybil set is therefore limited to the modeled $f$ Byzantine seats.
\end{itemize}
...
\begin{itemize}
    ...
    \item With one Byzantine replica under strategic rejection, A2A-Sim decides fewer tasks ($62.2\%$ vs.\ $74.4\%$) and commits wrong answers in up to $35.5\%$ of code tasks, whereas A2A-BFT commits no wrong answer on execution-validated code ($0.0\%$).
    \item The collusion cell in Table~\ref{tab:hetero_compare} is the one place where the comparison does not favour us: at $n{=}8, f{=}2, s{=}1$ on GSM8K, A2A-Sim's decision rate is marginally higher ($66.7\%$ vs.\ $61.1\%$) and its wrong-commit rate marginally lower ($7.8\%$ vs.\ $10.0\%$). Both differences are well within the seed-level variability observed in comparable cells (e.g., $7.8{\pm}1.9$ vs.\ $4.5{\pm}3.9$ for wrong-commit under strategic rejection; descriptive at three seeds), and neither survives pooling: what separates the methods is the code domain ($34.4$--$35.5\%$ vs.\ $0.0\%$ wrong commits) and the fact that A2A-BFT's decisions carry a proven guarantee rather than a confidence threshold. Under collusion the protocol is deliberately conservative---it forgoes a decision when validation is inconclusive---which costs decision rate in exactly the cell where an unvalidated aggregator happens to agree with itself.
\end{itemize}

\textbf{Design Philosophy:} While A2A-Sim focuses on evaluating LLM agent communication patterns, A2A-BFT addresses the lack of Byzantine fault tolerance in A2A networks. The two approaches are complementary: A2A-Sim provides insights into LLM consensus behavior, while A2A-BFT provides the theoretical guarantees necessary for production deployment.
...
\begin{enumerate}
    ...
    \item \textbf{LLM Parameter Sensitivity:} The $p_h \geq 0.9$ assumption is idealized: the measured judge accuracy is $\approx 0.80$ on GSM8K (Appendix~\ref{app:judge_calib}), and sensitivity curves across a range of $p_h$ values are future work.
...
A2A-BFT provides a foundation for reliable multi-agent collaboration under adversarial conditions. The three-phase semantic consensus design balances expressiveness with formal guarantees, while the reputation analysis of Section~\ref{sec:rep_ablation} identifies how the trigger must be redesigned to address practical concerns about agent reliability.
...
A2A-Sim \citep{a2a_sim_2026} provided the first systematic evaluation of LLM agent consensus, revealing only a 41.6\% success rate even in benign, no-stake settings, with failures dominated by liveness loss. Their work highlighted the gap between theoretical BFT guarantees and practical LLM behavior. A2A addresses agent-to-agent interoperability; the complementary Model Context Protocol (MCP) \citep{mcp_2024} standardizes connections from LLM applications to data sources and tools, and is orthogonal to the consensus layer studied here.

Recent approaches have explored confidence-aware and robust aggregation mechanisms. CP-WBFT \citep{zheng2025rethinking} introduces confidence probes to weight agent contributions, while SAC (Self-Anchored Consensus) \citep{lee2026robust} proposes decentralized filter-and-refine aggregation with graph-robustness conditions for decentralized LLM agent networks. Multi-agent debate showed that structured argumentation can improve reasoning accuracy \citep{du2024improving,liang2023encouraging}, and Self-Consistency demonstrated that aggregating multiple reasoning paths reduces error rates \citep{wang2023self}. Surveys of LLM-based multi-agent systems \citep{guo2024survey} provide taxonomies of collaboration patterns; frameworks such as AutoGen \citep{wu2023autogen} and CAMEL \citep{li2023camel} study multi-agent conversation among LLM instances. However, these approaches pursue collaboration quality rather than A2A protocol-level safety, and do not state formal safety and liveness guarantees with attack-resistance experiments. Our work differs by providing formal safety and liveness guarantees for consensus in the presence of Byzantine and soft faults within the A2A protocol framework, together with an execution-validated safety metric (wrong-commit rate).

\subsubsection{Near-Neighbour Byzantine-Robust LLM Coordination}

Three further lines sit closest to our threat model. First, multi-agent consensus seeking among LLMs has been studied in \emph{benign} settings without adversarial agents \citep{chen2023macs}; A2A-Sim \citep{a2a_sim_2026} first introduced adversarial agents into this setting, and our protocol adds a commit rule with a proven boundary. Second, BlockAgents \citep{chen2024blockagents} anchors LLM agent coordination on a blockchain for Byzantine robustness, and DecentLLMs achieves Byzantine-robust decentralized coordination of LLM agents through evaluator-scored consensus \citep{jo2025byzantine}; both harden the \emph{message exchange}, whereas A2A-BFT hardens the \emph{commitment}---validation sits inside the vote-counting rule, so wrong commits are bounded by the validation oracle's error rate (Corollary~\ref{cor:oracle}), a property that, to our knowledge, none of these designs states. Third, failure taxonomies catalogue why LLM multi-agent systems fail in practice \citep{cemri2025why}, and benchmark evidence shows coordination quality degrading as the communication graph grows \citep{grotschla2025agentsnet}; our measured liveness and safety results give a protocol-level counterpart to these empirical catalogues.

\subsubsection{Protocols and Verification Infrastructure}

On the infrastructure side, surveys of agent interoperability protocols \citep{ehtesham2025survey} place A2A alongside MCP \citep{mcp_2024} and the Agent Network Protocol (ANP) as complementary standards; our contribution is orthogonal to this landscape---a consensus layer over any authenticated agent fabric; the W3C Decentralized Identifiers standard \citep{did_core} supplies the identity primitive assumed in our threat model. On verification, process- and outcome-based verifiers improve single-model reasoning by ranking or filtering candidates \citep{gsm8k,lightman2024verify}; A2A-BFT instead places verification \emph{inside} the commit rule, which converts validator reliability into a stated wrong-commit bound rather than a post-hoc selection heuristic.
