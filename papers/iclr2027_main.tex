...
\begin{table}[htbp]
\centering\small
\caption{BFT Performance Under Attack (real heterogeneous 4-model deployment; cells: decision\% / wrong-commit\%, mean$\pm$std over sampling seeds; $n{=}250$ per cell except the $n{=}5, f{=}1, s{=}1$ strategic-reject row, $n{=}90$, from Table~\ref{tab:ablation}, Appendix~\ref{app:additional}). Bold marks the \emph{best} (lowest) wrong-commit rate per column. $^\dag$: configuration below the safety boundary $3f{+}s{+}1$ ($n \geq 7$ required for $f{=}2$). $^{\ddag}$: smaller ablation deployment (three seeds); not directly comparable to the $n{=}250$ rows. MMLU results in Table~\ref{tab:mmlu_sweep}.}
\label{tab:bft}
...
Three findings emerge. \textbf{(i) Safety is exactly as strong as the validation oracle; the boundary is tight.} On MBPP, every configuration within the safety boundary commits a wrong answer in $\mathbf{0.0 \pm 0.0\%}$ of tasks ($n{=}250$/cell; zero events; per-cell $95\%$ Wilson upper bound $1.5\%$) regardless of attack, whereas all $n{=}5, f{=}2$ configurations below the boundary violate safety ($5.6$--$9.6\%$): $\theta_{accept} = 0$ lets two Byzantine ACCEPTs offset the honest REJECTs of a non-tampering primary's wrong proposal. \textbf{(ii) On semantic domains, eventual acceptance trades safety for liveness under sustained attack.} GSM8K wrong commits rise to $11.6$--$23.2\%$ within the boundary: a Byzantine primary submits a plausible-but-wrong answer, weak 8B judges accept it at non-negligible rates, and retries sample until one passes validation---the system \emph{does} decide ($74$--$91\%$) but inherits the judge's error rate. \textbf{(iii) Decision rates rise under attack} (up to $98\%$ vs.\ $59.2\%$ fault-free): Byzantine primaries inject extra proposals that eventually clear validation; we therefore report wrong commits as the primary safety metric.

\subsection{Real LLM Validation (Homogeneous Upper Bound)}
\label{sec:realapi}

As a complementary check we validated the protocol on 320 real DeepSeek-V2-Lite tasks (GSM8K, MMLU, MBPP). These homogeneous single-model runs let honest validators agree far more often; the resulting $100\%$ acceptance across all eight configurations ($291$ of $320$ completed; $29$ API/harness failures excluded, Appendix~\ref{app:realllm}), under strategic rejection and collusion alike, is therefore an \emph{upper bound} on decision rates, not a contradiction of the $55$--$74\%$ heterogeneous results. At the boundary configuration $n{=}8, f{=}2, s{=}1$ (150 GSM8K tasks per configuration, real DeepSeek API), the protocol maintains $98$--$100\%$ consensus under both attacks across two independent runs, with answer correctness within $2$ points of the fault-free baseline ($92.0$--$92.7\%$ vs.\ $94.0\%$), and the round-count gap vs.\ fault-free ($1.03$ vs.\ $2.89$--$2.92$) confirms genuine attack resistance (Welch $t = -13.1$ and $-13.9$, $p < 10^{-26}$; reproduced at $1.04$ vs.\ $2.38$--$2.43$, with per-task decision agreement across runs, McNemar exact $p = 0.25$). Details: Appendix~\ref{app:realllm}; scaling and MMLU sweeps: Appendix~\ref{app:additional}.

\subsection{Ablation Study}

To isolate each protocol component we ablate in the \emph{heterogeneous four-model} setting (Section~\ref{sec:setup} models, one per replica) on two domains with different validation semantics: \emph{GSM8K}, where Validate uses an independent LLM judge, and \emph{MBPP}, where Validate executes the submitted code against test assertions---in verifiable domains a wrong commit is mechanically impossible. Each scenario uses 30 tasks per seed over three seeds ($90$ each); Byzantine validators employ strategic rejection ($n{=}5, f{=}1, s{=}1$) or collusion ($n{=}8, f{=}2, s{=}1$). Soft faults exercise the abstention arm (ABSTAIN w.p.\ $0.3$; Appendix~\ref{app:softfault})---the stochastic-REJECT arm of Definition~\ref{def:fault} is analyzed, not injected. Table~\ref{tab:ablation} (Appendix~\ref{app:additional}) reports decision rates and wrong-commit rates.


The ablation yields four findings. \textbf{(i) View change is the core liveness component:} removing it collapses decisions from $74.4 \pm 8.4\%$ to $4.5 \pm 3.9\%$ (GSM8K, strategic rejection) and to $0\%$ (MBPP, both attacks)---blocked rounds never rotate the primary, confirming Theorem~\ref{thm:liveness}. \textbf{(ii) Semantic validation is the prerequisite for heterogeneous consensus:} under naive exact matching heterogeneous replicas almost never agree byte-for-byte, so proposals are rejected indefinitely and the code domain deadlocks ($0$--$1.1\%$ decisions). On GSM8K, dropping semantic validation \emph{lowers} wrong commits ($2.2 \pm 1.9\%$ vs.\ the full protocol's $4.5 \pm 3.9\%$; conditional on deciding, $4.0\%$ vs.\ $6.0\%$)---as Corollary~\ref{cor:oracle} predicts: the exact-match oracle rarely commits and so rarely errs, while the LLM-judge oracle commits more often ($54.4\% \to 74.4\%$) and inherits the judge's error rate; both bounded by their respective oracle errors. \textbf{(iii) Fault-adaptive thresholds prevent attack-induced deadlock:} a fixed majority threshold drives every strategic-rejection round on GSM8K into the pending region ($0.0 \pm 0.0\%$, all seeds), whereas fault-adaptive thresholds restore $74.4 \pm 8.4\%$. \textbf{(iv) Safety is oracle-bounded:} at most $10.0 \pm 3.3\%$ wrong commits on GSM8K and none on MBPP (per-cell $95\%$ Wilson interval $[0.0, 4.1]\%$ at $n{=}90$).

\textbf{Comparison with baselines under Byzantine attacks (Table~\ref{tab:hetero_compare}, Appendix~\ref{app:additional}; definitions: Appendix~\ref{app:attacks}).} Three observations follow (descriptive; three seeds per cell). \textbf{First, high decision rates are not safety.} All voting- and debate-based baselines decide on every task, yet under collusion commit incorrect answers in $16.7$--$63.4\%$: unvalidated majority voting converts Byzantine injections and honest errors into committed consensus; the full protocol forgoes them ($60$--$74\%$ under attack). \textbf{Second, unverified debate amplifies attacks:} LLM-Debate \citep{du2024improving} is the least safe on code ($63.4 \pm 5.8\%$ wrong commits under collusion): unverified debate lets Byzantine agents drag honest replicas toward corrupted solutions. \textbf{Third, the correct-when-decided gap matters:} conditioned on committing, A2A-BFT reaches $84$--$100\%$ answer correctness in all eight attacked cells (Tables~\ref{tab:ablation},~\ref{tab:hetero_compare}) vs.\ $37$--$88\%$ for baselines; vs.\ A2A-Sim, GSM8K wrong commits are within noise, but A2A-Sim decides fewer tasks ($62.2\%$ vs.\ $74.4\%$) and collapses on code ($34.4$--$35.5\%$ vs.\ our $0.0{\pm}0.0$).

\subsection{Reputation Tracker: Measured Behavior}\label{sec:rep_ablation}

The tracker cannot change a commit decision (Section~\ref{sec:reputation}); we measure \emph{what it reports}: logging every validation vote from the four-model deployment (120 runs, 457 scored rounds, 2{,}461 validation events), we replay them offline through Algorithm~\ref{alg:reputation} under four settings ($c$ from ground truth or heuristic; persistent or per-task-reset reputation; Appendix~\ref{app:rep_details}). Decision-neutrality is \emph{checked} rather than assumed: recomputing $\phi$ from the raw votes reproduces all $457$ recorded rounds; replaying the vote-and-threshold state machine reproduces all $120$ decisions; injecting reputation weights shifts $\phi$ in up to $421$ rounds and flips up to $5$ decisions, so the check can fail.

The measured behavior is negative. \textbf{(i) The aggressive tier fires on honest, correct votes:} across the four attacked cells, $1{,}186$ of $1{,}305$ aggressive penalties ($\rho_i \geq 0.7$) landed on honest validators whose votes were \emph{objectively correct}---the behavior Section~\ref{sec:reputation} specifies should be rewarded---and \emph{all 20 tasks} per cell had a mis-penalty. \textbf{(ii) As a Byzantine detector the tier fails:} precision $0.232$/$0.195$ (recall $1.000$) under strategic rejection but precision \emph{and} recall $0.000$ under collusion---colluders never reach $\rho = 0.7$; honest validators rejecting the same wrong proposals do. \textbf{(iii) The score ordering inverts:} honest validators finish \emph{below} Byzantine ones in both collusion cells ($0.337$ vs.\ $0.427$ on GSM8K; $0.300$ vs.\ $0.412$ on MBPP), and per-task reset does not repair this ($0.391$ vs.\ $0.761$), so the failure is structural, not a cold-start artifact. \textbf{(iv) The released $c$ carries negative information:} \texttt{\_estimate\_correctness} is a per-domain constant (always \texttt{False} on GSM8K, \texttt{True} on MBPP) agreeing with ground truth on $46.2\%$ of rounds---worse than the trivial always-\texttt{False} predictor ($74.6\%$).

This failure mode is known \citep{josang2002beta,josang2007survey,kamvar2003eigentrust}; we add its scale in a real LLM deployment and evidence that it is structural. No reported safety or liveness \emph{number} depends on the tracker---used strictly as a diagnostic---so the quantitative results stand, while the deterrent in Appendix~\ref{app:attack_resistance} is specified rather than realized. A repaired trigger needs a proposal-quality oracle and a key untied to the validator's own rejection ratio; released vote streams make any repair measurable (Appendix~\ref{app:rep_details}).



\textbf{Limitations.} A2A-BFT assumes DID-authenticated channels (Ed25519); consensus quality depends on LLM capability: fault-free decision rates ($55$--$59\%$ GSM8K, $11$--$15\%$ MMLU; Table~\ref{tab:mmlu_sweep}) show weaker semantic-validation reliability, and Theorem~\ref{thm:liveness}'s asymptotic guarantee is unreached within six rounds on MMLU. The implementation scales to $n \leq 10$; ablation cells ($n{=}90$) limit statistical power; the tracker is decision-neutral yet mis-specified (single seed); the MMLU collapse reflects model capability.

\section{Conclusion}

We presented A2A-BFT, a Byzantine fault-tolerant consensus protocol for agent collaboration with provable safety and liveness under $n \geq 3f + s + 1$ ($s \leq f$). Real-LLM evaluation ($7{,}750$-task sweep, ablations, baselines) shows liveness under every evaluated attack and theory-predicted safety: zero wrong commits on execution-validated tasks within the boundary, violations below it, oracle-bounded errors on semantic tasks. Two limits remain: the measured reputation tracker is mis-specified (Section~\ref{sec:rep_ablation}); realized liveness on weak oracles falls short (implications, privacy, related work: Appendix~\ref{sec:discussion}).
