...
\textbf{Motivating Example:} five agents must agree on a code solution while one is compromised (Byzantine fault) or intermittently erroneous (soft fault). Traditional BFT protocols assume deterministic replicas; LLM-based agents add stochastic behavior that requires new consensus mechanisms.
...
\textbf{Reproducibility:} All protocol-level fault-tolerance experiments share one heterogeneous four-model deployment with 5 seeds and 50 tasks per (configuration, seed, dataset), i.e., $n{=}250$ per cell (7{,}750 tasks over 31 cells); task sets are seeded per-seed shuffles over GSM8K \citep{gsm8k}, MBPP \citep{mbpp}, and MMLU \citep{mmlu}, reused across configurations for paired comparison. \textbf{No simulation:} every reported number comes from real LLM inference ($7{,}750$ fault sweep + $4{,}320$ ablation/baseline + $770$ API tasks); seeds, models, hardware, injection, and the reputation measurement are specified in the Reproducibility Statement and Appendix~\ref{app:attacks}.
...
Three findings emerge. \textbf{(i) Safety is exactly as strong as the validation oracle; the boundary is tight.} On MBPP, every configuration within the safety boundary commits a wrong answer in $\mathbf{0.0 \pm 0.0\%}$ of tasks ($n{=}250$/cell; zero events; per-cell $95\%$ Wilson upper bound $1.5\%$) regardless of attack, whereas all $n{=}5, f{=}2$ configurations below the boundary violate safety ($5.6$--$9.6\%$), where $\theta_{accept} = 0$ lets two Byzantine ACCEPTs offset the honest REJECTs of a non-tampering primary's wrong proposal. \textbf{(ii) On semantic domains, eventual acceptance trades safety for liveness under sustained attack.} GSM8K wrong commits rise to $11.6$--$23.2\%$ within the boundary: a Byzantine primary submits a plausible-but-wrong answer, weak 8B judges accept it at non-negligible rates, and retries keep sampling until one passes validation---the system \emph{does} decide ($74$--$91\%$) but inherits the judge's error rate. \textbf{(iii) Decision rates rise under attack} (up to $98\%$ vs.\ $59.2\%$ fault-free): Byzantine primaries inject extra proposals that eventually clear validation, so we report wrong commits as the primary safety metric.

\subsection{Real LLM Validation (Homogeneous Upper Bound)}
\label{sec:realapi}

As a complementary check we validated the protocol with real DeepSeek-V2-Lite outputs over 320 tasks (GSM8K, MMLU, MBPP). These homogeneous single-model runs let honest validators agree far more often than in the heterogeneous sweep; the resulting $100\%$ acceptance across all eight configurations ($291$ of $320$ tasks completed; $29$ API/harness failures excluded, Appendix~\ref{app:realllm}), under strategic rejection and collusion alike, is therefore an \emph{upper bound} on decision rates rather than a contradiction of the $55$--$74\%$ heterogeneous results. At the boundary configuration $n{=}8, f{=}2, s{=}1$ (150 GSM8K tasks per configuration, real DeepSeek API), the protocol maintains $98$--$100\%$ consensus under both attacks across two independent runs, with answer correctness within $2$ points of the fault-free baseline ($92.0$--$92.7\%$ vs.\ $94.0\%$), and the round-count gap vs.\ fault-free ($1.03$ vs.\ $2.89$--$2.92$) confirms genuine attack resistance (Welch $t = -13.1$ and $-13.9$, $p < 10^{-26}$; reproduced at $1.04$ vs.\ $2.38$--$2.43$, with per-task decision agreement across runs, McNemar exact $p = 0.25$). Details: Appendix~\ref{app:realllm}; scaling and MMLU sweeps: Appendix~\ref{app:additional}.
...
Three observations follow (descriptive; three seeds per cell). \textbf{First, high decision rates are not safety.} All voting- and debate-based baselines decide on every task, yet under collusion commit incorrect answers in $16.7$--$63.4\%$: unvalidated majority voting converts Byzantine injections and honest errors into committed consensus, whereas the full protocol forgoes such decisions ($60$--$74\%$ decisions under attack). \textbf{Second, unverified debate amplifies attacks:} LLM-Debate \citep{du2024improving} is the least safe on code ($63.4 \pm 5.8\%$ wrong commits under collusion), because unverified debate lets Byzantine agents drag honest replicas toward corrupted solutions. \textbf{Third, the correct-when-decided gap matters:} conditioned on committing, A2A-BFT reaches $84$--$100\%$ answer correctness across all eight attacked cells (Tables~\ref{tab:ablation},~\ref{tab:hetero_compare}) versus $37$--$88\%$ for the baselines; against A2A-Sim, our GSM8K wrong-commit rates are within noise (descriptive only at three seeds), but A2A-Sim decides fewer tasks ($62.2\%$ vs.\ $74.4\%$) and collapses on code ($34.4$--$35.5\%$ vs.\ our $0.0{\pm}0.0$).

\textbf{Note on evaluation budget.} Baselines commit in a single round whereas A2A-BFT retries for up to six rounds; under the same budget, the oracle-assisted debate baseline still commits wrong answers on $46.7$--$56.7\%$ of MBPP tasks, so the safety gap is not a pure budget effect (Appendix~\ref{app:attacks}).

\subsection{Reputation Tracker: Measured Behavior}\label{sec:rep_ablation}

As established in Section~\ref{sec:reputation}, the tracker cannot change a commit decision; we instead measure \emph{what the tracker reports}: we logged every validation vote from the four-model deployment (120 runs, 457 scored proposal rounds, 2{,}461 validation events) and replayed them offline through Algorithm~\ref{alg:reputation} under four settings ($c$ from ground truth or released heuristic; reputation persisting or per-task reset; Appendix~\ref{app:rep_details}). Decision-neutrality is \emph{checked} rather than assumed: recomputing $\phi$ from the raw votes reproduces all $457$ recorded rounds; replaying the vote-and-threshold state machine reproduces all $120$ decisions with primary rotation; injecting reputation weights shifts $\phi$ in up to $421$ rounds and flips up to $5$ decisions, so the check can fail (Appendix~\ref{app:rep_details}).

The measured behavior is negative. \textbf{(i) The aggressive tier fires on honest, correct votes:} across the four attacked cells, $1{,}186$ of $1{,}305$ aggressive penalties ($\rho_i \geq 0.7$) landed on an honest validator whose vote was \emph{objectively correct}---the behavior that Section~\ref{sec:reputation} specifies should be rewarded---and \emph{all 20 tasks} in each attacked cell produced at least one mis-penalty. \textbf{(ii) As a Byzantine detector the tier fails:} precision $0.232$/$0.195$ (recall $1.000$) under strategic rejection but precision \emph{and} recall $0.000$ under collusion---a colluder rejects only in the commit round, never reaching $\rho = 0.7$, whereas honest validators rejecting the same wrong proposals do. \textbf{(iii) The score ordering inverts:} honest validators finish \emph{below} Byzantine ones in both collusion cells ($0.337$ vs.\ $0.427$ on GSM8K; $0.300$ vs.\ $0.412$ on MBPP), and resetting reputation per task does not repair this ($0.391$ vs.\ $0.761$), so the failure is structural rather than a cold-start artifact. \textbf{(iv) The released $c$ carries negative information:} \texttt{\_estimate\_correctness} is a per-domain constant (always \texttt{False} on GSM8K, \texttt{True} on MBPP) agreeing with ground truth on $46.2\%$ of rounds---worse than the trivial always-\texttt{False} predictor ($74.6\%$)---so the promised reward for correct rejection never occurs.

This failure mode is known to the trust literature \citep{josang2002beta,josang2007survey,kamvar2003eigentrust}; what we add is its scale in a real LLM deployment and evidence that it is structural. Because no reported safety or liveness \emph{number} depends on the tracker, the quantitative results stand, but the deterrent in Appendix~\ref{app:attack_resistance} is specified rather than realized. A repaired trigger needs a proposal-quality oracle and a key not tied to the validator's own rejection ratio (Appendix~\ref{app:rep_details}); released vote streams make any repair measurable the same way.



\textbf{Limitations.} A2A-BFT assumes DID-authenticated channels (Ed25519); consensus quality depends on LLM capability: fault-free decision rates ($55$--$59\%$ GSM8K, $11$--$15\%$ MMLU; Table~\ref{tab:mmlu_sweep}) show weaker validation reliability on semantic domains, and Theorem~\ref{thm:liveness}'s asymptotic guarantee is not reached within six evaluated rounds on MMLU. The implementation scales to $n \leq 10$; ablation cells ($n{=}90$) limit statistical power; the tracker is decision-neutral but mis-specified (single seed); the MMLU collapse reflects model capability, not the protocol; privacy: Appendix~\ref{sec:discussion}.

\section{Conclusion}

We presented A2A-BFT, a Byzantine fault-tolerant consensus protocol for Agent-to-Agent collaboration with provable safety and liveness under $n \geq 3f + s + 1$ ($s \leq f$). Real-LLM evaluation ($7{,}750$-task fault sweep plus ablations and baselines) shows liveness under every evaluated attack and safety that binds where the theory predicts: zero wrong commits on execution-validated tasks within the boundary, violations below it, and oracle-bounded errors on semantic tasks. Two limits remain: the measured reputation tracker is mis-specified (Section~\ref{sec:rep_ablation}), and realized liveness on weak oracles falls short of the guarantee. Implications, privacy, and related work: Appendix~\ref{sec:discussion}.
