...
- 五张图的**存在性**与**新鲜度**（内容指纹口径：`figures/.figsource.json` 记录
  ...
  两种指纹都**按构造即平台中性**：数据 md5 先做 CRLF→LF 规范化再算，逻辑指纹用
  自建 AST 编码而非 `ast.dump()`（后者的输出格式在 Python 3.13 变了）。否则在
  Linux 上普通 `git clone` 会让五张图全部误报陈旧——2026-09-20 实测：记录的数据
  md5 就是 CRLF 字节串，且同一份生成脚本在 3.13 下得 `3489a022`、3.12 下得 `a671116d`
