...
class A2ABFT:
    ...
    def _validate(self, w, task, proposal, primary_is_byzantine):
        ...
        if w.task_type == 'code':
            from multi_model_vllm import extract_code
            po = run_code_tests(extract_code(own), task.get('test_list', []))
            pp = run_code_tests(extract_code(proposal), task.get('test_list', []))
            if not po and not pp:
                return 'ABSTAIN'
            return 'ACCEPT' if po == pp else 'REJECT'
...
def llm_debate(workers, task, task_type, rounds=2):
    ...
    for _ in range(rounds):
        ...
        answers = new
    cand = [normalize(a, task, task_type) for a in answers]
...
def main():
    print('=' * 78)
    print('4模型异构：主算法 / 对比算法 / 消融算法 v2 (30任务 x 6场景)')
    ...
    scenarios = [
        {'dataset': 'gsm8k', 'n': 5, 'f': 0, 's': 0, 'attack': None, 'tasks': 30},
        {'dataset': 'gsm8k', 'n': 5, 'f': 1, 's': 1, 'attack': 'strategic_reject', 'tasks': 30},
        {'dataset': 'gsm8k', 'n': 8, 'f': 2, 's': 1, 'attack': 'collusion', 'tasks': 30},
    ...
    for sc in scenarios:
        ...
        part = os.path.join(RESULTS_DIR, 'multi_model_compare_ablation_v2.json')
    ...
    out = os.path.join(RESULTS_DIR, 'multi_model_compare_ablation_v2.json')
    ...
    print('=' * 100)
    print('COMPARE_ABLATION_V2_DONE')
