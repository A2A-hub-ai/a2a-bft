...
# 归一化 / 判定
# ----------------------------------------------------------------------------
def normalize(ans: str, task_type: str):
    if not ans:
        return None
    if task_type == 'math':
...
def a2a_sim(workers, task, task_type, conf_th=0.7):
    ...
    return top if k > len(cand) / 2 else None
    ...
    for _ in range(rounds):
        ...
        for i, w in enumerate(workers):
            if w.is_byzantine:          # 拜占庭节点不改变立场
                ...
                continue
            others = [normalize(a, task_type) for j, a in enumerate(answers) if j != i]
            ...
            new.append(raw if normalize(raw, task_type) else answers[i])
        answers = new
    cand = [normalize(a, task_type) for a in answers]
...
def main():
    ...
    scenarios = [
        {'dataset': 'gsm8k', 'n': 5, 'f': 0, 's': 0, 'attack': None, 'tasks': 10},
        {'dataset': 'gsm8k', 'n': 5, 'f': 1, 's': 1, 'attack': 'strategic_reject', 'tasks': 10},
        {'dataset': 'gsm8k', 'n': 8, 'f': 2, 's': 1, 'attack': 'collusion', 'tasks': 10},
    ]
    ...
    baselines = [
        ('Simple Majority', lambda ws, t: simple_majority(ws, t, 'math')),
        ('Weighted Majority', lambda ws, t: weighted_majority(ws, t, 'math')),
        ('A2A-Sim', lambda ws, t: a2a_sim(ws, t, 'math')),
        ('LLM-Debate', lambda ws, t: llm_debate(ws, t, 'math', rounds=2)),
    ]

    all_rows = []
    for sc in scenarios:
        ds, n, f, s, atk = sc['dataset'], sc['n'], sc['f'], sc['s'], sc['attack']
        task_type = 'math' if ds == 'gsm8k' else 'knowledge'
        ...
        print(f"\n{'#'*78}")
        print(f"# 场景: {ds}  n={n}, f={f}, s={s}, attack={atk or 'baseline'}")
    ...
    out = os.path.join(RESULTS_DIR, 'multi_model_compare_ablation.json')
